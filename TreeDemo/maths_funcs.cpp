...
// returns an orthographic function mimicing the opengl projection style. COLUMN ORDER
mat4 ortho(float left, float right, float bottom, float top, float near, float far)
{

	mat4 m = zero_mat4();
	float o1 = 2 / (right - left);
	float o2 = 2 / (top - bottom);
	float o3 = -(2/ (far - near));
	float o4 = -((left+right)/(right-left));
	float o5 = -((top+bottom)/(top-bottom));
	float o6 = -((far+near)/(far-near));
